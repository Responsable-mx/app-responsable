...
- [B] ✅ **DM-IA navegación mejorada — Ruta B** — Solo 1 etapa en DOM a la vez (CollapsibleStageSection retorna null si no activa). Stepper con role="tablist", StagePill con role="tab" + aria-selected + tabIndex, sección con role="tabpanel" + aria-labelledby. Hash routing + back/forward + smart-jump + teclado ←→ activos. may-2026
...
- [B] ✅ **Módulo costos/pricing** — `service_pricing_config` (costo base maduro por tipo de servicio), `client_services.{is_pilot,actual_cost,sale_price,cost_notes}` (costo real en **USD** por entrega). Admin configura precios base en `/configuracion/costos`. Cada servicio en cronograma muestra `ProjectCostCard` con toggle piloto · costo real · precio venta · margen · desviación vs. referencia. API `GET/POST /api/service-pricing` · `PATCH /api/client-services/[id]` extendido. Migración `0093`. may-2026.
- [B] ✅ **Costo por etapa DM-IA** — `GET /api/clients/[id]/ai-costs` agrupa `ai_calls` por `workflow_stage` (dm_referentes/dm_benchmark/dm_iros/etc.), calcula costo por modelo, devuelve array. `PricingConfigTable` muestra "▼ ver etapas" lazy por proyecto. `auditoria-ia` divide tabla by_stage en DM-IA vs Chat. may-2026.
