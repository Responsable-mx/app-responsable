...
function TabButton({
  ...
  return (
    <button
      ...
      title={label}
      onClick={onClick}
      className={`px-3 py-2.5 border-b-2 -mb-px transition-colors flex items-center gap-1 whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary/40 focus-visible:ring-inset ${
        active
          ? "border-brand-primary text-brand-primary-dark"
          : "border-transparent text-slate-400 hover:text-slate-700"
      }`}
    >
      <span className={`${active ? "text-brand-primary-dark" : "text-slate-400"} w-[18px] h-[18px] flex items-center justify-center`}>{icon}</span>
      {badge !== null && (
        <span
          title={badgeTitle}
