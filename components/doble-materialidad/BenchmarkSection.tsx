...
import { CRITERIO_LABELS } from "@/lib/dm/benchmark-empresas-types";
...
export function BenchmarkSection({
  ...
  const [importingReferentes, setImportingReferentes] = useState(false);
  const [referentesExpanded, setReferentesExpanded] = useState(false);

  const handleImportFromReferentes = useCallback(async () => {
    setImportingReferentes(true);
    try {
      ...
      if (!res.ok) throw new Error(json.error ?? "Error al importar empresas");
      const { imported, skipped } = json.data as { imported: number; skipped: number };
      push(
        "success",
        imported > 0
          ? `${imported} empresa${imported !== 1 ? "s" : ""} importada${imported !== 1 ? "s" : ""} desde Etapa 3.${skipped > 0 ? ` ${skipped} ya existían.` : ""}`
          : "Todas las empresas de Etapa 3 ya estaban en el benchmark."
      );
      onDataMutate();
    } catch (e) {
      push("error", e instanceof Error ? e.message : "Error al importar empresas");
    } finally {
      setImportingReferentes(false);
    }
    ...
    <div className="space-y-4">
      ...
      {hasDone && !configExpanded ? (
        ...
        <div className="space-y-4">
          ...
          {/* Fuente de empresas: Etapa 3 (referentes) o propuesta IA */}
          {hasReferentes ? (
            /* ── Modo Etapa 3 — ocultar "Proponer con IA" ── */
            <div className="space-y-3">
              {/* Banner informativo Etapa 3 */}
              <div className="flex items-start gap-3 px-3 py-2.5 bg-cyan-50 border border-cyan-200 rounded">
                <svg className="w-3.5 h-3.5 mt-0.5 shrink-0 text-cyan-600" viewBox="0 0 16 16" fill="currentColor">
                  <path fillRule="evenodd" d="M8 15A7 7 0 108 1a7 7 0 000 14zm.75-10.25a.75.75 0 00-1.5 0v4.5a.75.75 0 001.5 0v-4.5zM8 11a.75.75 0 100 1.5.75.75 0 000-1.5z" clipRule="evenodd" />
                </svg>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-cyan-800">
                    {referentCompanies.length} empresa{referentCompanies.length !== 1 ? "s" : ""} de referencia validadas en Etapa 3
                  </p>
                  <p className="text-[10px] text-cyan-700 mt-0.5">
                    Importa las empresas de la Etapa anterior para ejecutar el análisis comparativo.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setReferentesExpanded((v) => !v)}
                  className="text-[10px] text-cyan-600 hover:text-cyan-800 underline shrink-0"
                >
                  {referentesExpanded ? "Ocultar" : "Ver lista"}
                </button>
              </div>

              {/* Lista colapsable de empresas de Etapa 3 */}
              {referentesExpanded && (
                <div className="border border-slate-200 rounded divide-y divide-slate-100">
                  {referentCompanies.map((e) => (
                    <div key={e.id} className="flex items-center gap-2 px-3 py-2">
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-slate-800">{e.nombre}</p>
                        <p className="text-[10px] text-slate-500">
                          {e.pais}{e.subsector ? ` · ${e.subsector}` : ""}
                        </p>
                      </div>
                      <span className="text-[9px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded-sm font-semibold shrink-0 whitespace-nowrap">
                        {CRITERIO_LABELS[e.criterio]?.split(" — ")[0] ?? e.criterio}
                      </span>
                    </div>
                  ))}
                </div>
              ...
              {/* Botones: importar + agregar manual */}
              <div className="flex items-center gap-3 flex-wrap">
                <Button
                  size="sm"
                  variant={companies.length > 0 ? "secondary" : "primary"}
                  loading={importingReferentes}
                  onClick={() => void handleImportFromReferentes()}
                >
                  <svg className="w-3 h-3 mr-1.5 shrink-0" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M6 1v7M6 8l-2.5-2.5M6 8l2.5-2.5M1 11h10" />
                  </svg>
                  {companies.length > 0 ? "Re-importar desde Etapa 3" : "Importar empresas de Etapa 3"}
                </Button>
                <button
                  type="button"
