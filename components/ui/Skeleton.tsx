  ...
  return (
    <div className="flex items-center gap-4 p-4 animate-pulse">
      ...
      {Array.from({ length: cols - 1 }).map((_, i) => (
        <div key={i} className="h-4 bg-slate-100 rounded w-20 shrink-0" />
      ))}
    </div>
  );
}
...
export function SkeletonTable({
  ...
  return (
    <div className="bg-white rounded shadow-sm border border-slate-100 overflow-hidden divide-y divide-slate-50">
  ...
  return (
    <div className="space-y-4 animate-pulse">
      {Array.from({ length: items }).map((_, i) => (
        <div
          key={i}
          className={`bg-white rounded border border-slate-200 ${itemHeight}`}
        />
      ))}
    </div>
  );
...
export function SkeletonDetail() {
  return (
    <div className="bg-white rounded shadow-sm p-6 space-y-4 animate-pulse">
